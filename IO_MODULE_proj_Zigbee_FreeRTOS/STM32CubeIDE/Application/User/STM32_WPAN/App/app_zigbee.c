...
void APP_ZIGBEE_GetThermAddr(void);
//void APP_ZIGBEE_GetThermTemp(void);

void Zigbee_Therm_Addr_Start(void);
void Zigbee_Temp_Start(void);
...
osSemaphoreId_t       AddressSemaphore;
...
uint16_t ZigbeeCurrentTemp[4]={ 273,  273,  273,  273 };
uint16_t ZigbeeSetpointTemp[4]={ 273,  273,  273,  273 };
uint8_t Zigbee_ready = 0;
...
void APP_ZIGBEE_Init(void)
{
  ...
  /*ADD ON*/
   ...
   /* Task associated with getting thermostat short network address */
...
static void APP_ZIGBEE_StackLayersInit(void)
{
  //vTaskSuspend(ModbusH.myTaskModbusAHandle);
  ...
  zigbee_app_info.currentTemp = 273;
  zigbee_app_info.minTemp = 0;
  zigbee_app_info.maxTemp = 0;
  zigbee_app_info.setpointTemp = 0;
...
static void APP_ZIGBEE_NwkForm(void)
{
  ...
  else
  {
    ...
    //run the task to get short address of thermostat
    osThreadFlagsSet(OsTaskGetThermAddr,1);
    //APP_DBG ("run temp task")the ;
...
static void APP_ZIGBEE_CheckWirelessFirmwareInfo(void)
{
  ...
  APP_DBG("call getwireless");
  while (SHCI_GetWirelessFwInfo(p_wireless_info) != SHCI_Success)
  {
...
static void APP_ZIGBEE_ProcessGetThermAddr(void *argument)
{
  ...
  for(;;)
  {
	//flag = osEventFlagsWait(nwk_event_id, FLAG_NWK, osFlagsWaitAll, osWaitForever);
	osThreadFlagsWait(1,osFlagsWaitAny,osWaitForever);
    APP_ZIGBEE_GetThermAddr();
    //if (zigbee_app_info.thermShortAddr !=0)
    //{
	//	osEventFlagsSet(asoc_event_id, FLAG_ASOC);
    //}
  }
}
...
void APP_ZIGBEE_GetThermAddr(void)
{
	...
	{
		APP_DBG("Thermostat short address requested");
	}
...
static void GetThermAddr_cb(struct ZbZdoNwkAddrRspT  *rsp, void *cb_arg)
	{
		if (rsp->status == ZB_STATUS_SUCCESS)
		{
		    ...
		    APP_DBG("zigbee_app_info.thermextaddr extAddr = 0x%04x",  zigbee_app_info.thermExtAddr);
			...
			//osThreadFlagsSet(OsTaskGetThermTemp,1);
			//if thermostat has not joined the network yet, run the short address task again
			if (zigbee_app_info.thermShortAddr == 0x000U)
				{
					osThreadFlagsSet(OsTaskGetThermAddr,1);
				}

			//if thermostat has joined the network yet, release the  modbus task
			if (zigbee_app_info.thermShortAddr != 0x000U)
			{
				//osThreadFlagsSet(OsTaskGetThermTemp,1);
		...
		//if network has not been formed yet (callback function fails), run the short address task again
		else
		{
  ...
  for(i=0;i<readRsp->count;i++)
  {
    if(readRsp->attr[i].status == ZCL_STATUS_SUCCESS)
    {
      ...
      case ZCL_THERM_SVR_ATTR_LOCAL_TEMP:
        currentTemp = pletoh16(readRsp->attr[i].value);
        APP_DBG("[DEV TEMP]Current Temperature= %d,%dC",currentTemp/100, currentTemp%100);
        zigbee_app_info.currentTemp = pletoh16(readRsp->attr[i].value);
        ZigbeeCurrentTemp[0]= pletoh16(readRsp->attr[i].value);
        break;
      case ZCL_THERM_SVR_ATTR_OCCUP_COOL_SETPOINT:
    	minTemp = pletoh16(readRsp->attr[i].value);
        APP_DBG("[DEV TEMP] Min set Temperature= %d,%dC",minTemp/100,minTemp%100);
        if (minTemp != zigbee_app_info.minTemp)
        {
        	zigbee_app_info.minTemp = pletoh16(readRsp->attr[i].value);
        	zigbee_app_info.setpointTemp = pletoh16(readRsp->attr[i].value);
        	ZigbeeSetpointTemp[0]= pletoh16(readRsp->attr[i].value);
        	APP_DBG("[DEV TEMP] New setpoint Temperature= %d,%dC",minTemp/100,minTemp%100);
        }
        break;
      case ZCL_THERM_SVR_ATTR_OCCUP_HEAT_SETPOINT:
    	maxTemp = pletoh16(readRsp->attr[i].value);
        APP_DBG("[DEV TEMP] Max set Temperature= %d,%dC",maxTemp/100,maxTemp%100);
        if (maxTemp != zigbee_app_info.maxTemp)
        {
        	zigbee_app_info.maxTemp = pletoh16(readRsp->attr[i].value);
        	zigbee_app_info.setpointTemp = pletoh16(readRsp->attr[i].value);
        	ZigbeeSetpointTemp[0]= pletoh16(readRsp->attr[i].value);
        	APP_DBG("[DEV TEMP] New setpoint Temperature= %d,%dC",maxTemp/100,maxTemp%100);
        }
        break;
      default:
        break;
...
//
/* Thread for reading Zigbee thermostat address data*/
void Zigbee_Therm_Addr_Start()
{
	OsTaskGetThermAddr = osThreadNew(APP_ZIGBEE_ProcessGetThermAddr, NULL,&TaskGetThermAddr_attr);
}

/* Thread for reading Zigbee temp data*/
//void Zigbee_Temp_Start()
//{
//	OsTaskGetThermTemp = osThreadNew(APP_ZIGBEE_ProcessGetThermTemp, NULL,&TaskGetThermTemp_attr);
//}
