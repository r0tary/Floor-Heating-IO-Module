...
// Zigbee Temperature thread handle
osThreadId_t ZigbeeTempReadHandle;
const osThreadAttr_t ZigbeeTempRead_attributes = {
  .name = "ZigbeeTempRead",
  .priority = (osPriority_t) osPriorityBelowNormal7,
  .stack_size = 128 * 4
};
...
void IO_Module_Init(io_module_t * IO)
{
	...
	*/
	...
	for(int i = 0; i < N_ROOMS; i++)
	{
		...
		rooms[i].ThermTempRoom = 273;
		rooms[i].ThermSetpointTemp = 273;
...
{
	ZigbeeTempReadHandle = osThreadNew(TempZigbeeRead, IO, &ZigbeeTempRead_attributes);
...
void TempZigbeeRead(void *argument){
	io_module_t *IO = (io_module_t *)argument;
	uint16_t *temp;
	for(;;)
	{
		osThreadFlagsWait(0x01, osFlagsWaitAny, osWaitForever);
		//osSemaphoreAcquire(ModbusH.ModBusSphrHandle, osWaitForever);
		//wait for Modbus to realease USART Mutex
		osMutexAcquire(MtxUSART, osWaitForever);
		//set Rx Callback type flag
		RxCB_type = 0;
		//request Zigbee Thermostat readings
		...
		//realease USART Mutex
		osMutexRelease(MtxUSART);
		//osSemaphoreRelease(ModbusH.ModBusSphrHandle); //Release the semaphore
		osDelay(1);
	}
}
