...
osThreadId_t ModbusStartHandle;
...
const osThreadAttr_t  ModbusStart_attributes = {
  .name = "ModbusStart",
  .priority = (osPriority_t) osPriorityNormal,
  .stack_size = 128 * 4
};
...
void StartModbusInitialization(void *argument);
...
/* Configure the peripherals common clocks */
  ...
  //Zigbee_Therm_Addr_Start();

  //Zigbee_Temp_Start();


  IOmodule.u16regsHR = Holding_Registers_Database;
  IOmodule.u16regsRO = Input_Registers_Database;
  ...
  ModbusH.ModbusInit = 0;
  ModbusH.ModbusStart = 0;


  /* Initialize the mutex */
  MtxUSART = osMutexNew( NULL );
  ...
  ModbusStartHandle = osThreadNew(StartModbusInitialization, NULL, &ModbusStart_attributes);
  /* USER CODE BEGIN RTOS_THREADS */
  /* add threads, ... */
  ADC_Temp_Thread_Start(&IOmodule);
  Zigbee_Temp_Thread_Start(&IOmodule);
