...
static void APP_ZIGBEE_EP_cb(struct ZbZdoActiveEpRspT *rsp, void *cb_arg){
	if (rsp->status == ZB_STATUS_SUCCESS) {
		APP_DBG("%s : Callback success = 0x%02x", __func__, rsp->status);
		return;
	}
/**
 * @brief  Zigbee application initialization
...
//input for EP request
    uint16_t newTermostatAddress = 0x0000U;
    struct ZbZdoActiveEpReqT ActiveEPReq;
	memset(&ActiveEPReq, 0, sizeof(ActiveEPReq));
	ActiveEPReq.nwkAddrOfInterest = newTermostatAddress;
	ActiveEPReq.dstNwkAddr = newTermostatAddress;
